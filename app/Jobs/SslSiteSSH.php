...
class SslSiteSSH implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $ssh->exec("echo " . $this->site->server->password . " | sudo -S sudo sed -i 's/443 ssl/443 ssl http2/g' /etc/nginx/sites-available/" . $this->site->username . ".conf");
        ...
        foreach ($this->site->aliases as $alias) {
            ...
            $ssh->exec("echo " . $this->site->server->password . " | sudo -S sudo sed -i 's/443 ssl/443 ssl http2/g' /etc/nginx/sites-available/" . $alias->domain . ".conf");
            $ssh->exec('echo ' . $this->site->server->password . ' | sudo -S sudo ufw --force enable');
            $ssh->exec('echo ' . $this->site->server->password . ' | sudo -S sudo systemctl restart nginx.service');
        }
        $ssh->exec('exit');
